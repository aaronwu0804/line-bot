#!/bin/bash
# filepath: /Users/al02451008/Documents/code/morning-post/deploy_robust_fix.sh
# 部署強化版關鍵字檢測修復至 Render
...
echo -e "${YELLOW}=== 部署強化版關鍵字檢測修復 ===${NC}"
echo -e "${BLUE}此版本增加了更頑健的關鍵字檢測方法${NC}"
echo -e "${BLUE}並添加了更詳細的診斷日誌以幫助排查問題${NC}"
...
if [ "$COMMIT_CHANGES" = "y" ] || [ "$COMMIT_CHANGES" = "Y" ]; then
    ...
    git add app.py src/line_webhook.py keyword_debug.py
    git commit -m "增強關鍵字檢測能力，添加更頑健的處理方式"
...
echo ""
echo -e "${BLUE}1. 訪問 Render 儀表板: ${PURPLE}https://dashboard.render.com${NC}"
...
echo -e "${GREEN}1. 發送帶有「花生」的訊息: ${NC}花生 香蕉的英文是什麼"
echo -e "${GREEN}2. 發送帶有「小幫手」的訊息: ${NC}小幫手 告訴我今天的天氣"
echo -e "${GREEN}3. 測試連續對話: ${NC}先發送有關鍵字的訊息，再直接發送問題"
echo -e "${GREEN}4. 測試結束對話: ${NC}發送「結束對話」"
echo ""
echo -e "${PURPLE}部署後請查看 Render 日誌，確認關鍵字檢測是否有效${NC}"

echo -e "${YELLOW}如需進一步診斷問題，可執行關鍵字檢測診斷工具:${NC}"
echo -e "${BLUE}python3 keyword_debug.py${NC}"
